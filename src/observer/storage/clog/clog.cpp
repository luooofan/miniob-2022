...
#define CLOG_UPS_REC_NODATA_SIZE (sizeof(CLogUpdateRecord) - sizeof(char *))
  ...
  switch (flag) {
    ...
    case REDO_UPDATE: {
      if (!rec || !rec->data()) {
        ...
        log_record_.ups.hdr_.trx_id_ = trx_id;
        log_record_.ups.hdr_.type_ = flag;
        strcpy(log_record_.ups.table_name_, table_name);
        log_record_.ups.rid_ = rec->rid();
        log_record_.ups.data_len_ = data_len;
        log_record_.ups.hdr_.logrec_len_ = _align8(CLOG_UPS_REC_NODATA_SIZE + data_len);
        log_record_.ups.data_ = new char[log_record_.ups.hdr_.logrec_len_ - CLOG_UPS_REC_NODATA_SIZE];
        memcpy(log_record_.ups.data_, rec->data(), data_len);
        log_record_.ups.hdr_.lsn_ = CLogManager::get_next_lsn(log_record_.ups.hdr_.logrec_len_);
      }
    } break;
    case REDO_DELETE: {
      if (!rec) {
...
CLogRecord::CLogRecord(char *data)
{
  ...
  switch (flag_) {
    ...
    case REDO_UPDATE: {
      log_record_.ups.hdr_ = *hdr;
      data += sizeof(CLogRecordHeader);
      strcpy(log_record_.ups.table_name_, data);
      data += TABLE_NAME_MAX_LEN;
      log_record_.ups.rid_ = *(RID *)data;
      data += sizeof(RID);
      log_record_.ups.data_len_ = *(int *)data;
      data += sizeof(int);
      log_record_.ups.data_ = new char[log_record_.ups.hdr_.logrec_len_ - CLOG_UPS_REC_NODATA_SIZE];
      memcpy(log_record_.ups.data_, data, log_record_.ups.data_len_);
    } break;
    case REDO_DELETE: {
...
RC CLogRecord::copy_record(void *dest, int start_off, int copy_len)
{
  ...
  if (start_off + copy_len > get_logrec_len()) {
    ...
    if (start_off > CLOG_UPS_REC_NODATA_SIZE) {
      memcpy(dest, log_rec->ups.data_ + start_off - CLOG_UPS_REC_NODATA_SIZE, copy_len);
    } else if (start_off + copy_len <= CLOG_UPS_REC_NODATA_SIZE) {
      memcpy(dest, (char *)log_rec + start_off, copy_len);
    } else {
      memcpy(dest, (char *)log_rec + start_off, CLOG_UPS_REC_NODATA_SIZE - start_off);
      memcpy((char *)dest + CLOG_UPS_REC_NODATA_SIZE - start_off,
          log_rec->ups.data_,
          copy_len - (CLOG_UPS_REC_NODATA_SIZE - start_off));
    }
  }
  return RC::SUCCESS;
}
...
int CLogRecord::cmp_eq(CLogRecord *other)
{
  ...
  if (flag_ == other->flag_) {
    switch (flag_) {
      ...
      case REDO_UPDATE:
        return log_record_.ups == other_logrec->ups;
      case REDO_DELETE:
