...
UpdateStmt::UpdateStmt(Table *table, std::vector<char *> attr_names, std::vector<Value> values, FilterStmt *filter_stmt)
...
RC UpdateStmt::create(Db *db, const Updates &update, Stmt *&stmt)
{
  ...
  for (size_t i = 0; i < update.attribute_num; i++) {
    ...
    if (nullptr == attr_name) {
      LOG_WARN("invalid argument. field_name=%p", attr_name);
      return RC::INVALID_ARGUMENT;
    }
    ...
    attr_names.push_back(const_cast<char *>(attr_name));
